...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        temp_out_diff1 = func.interpolate(temp_out_diff1, x.size()[2:])
        x = self.apha*x + self.belta*temp_out_diff1
        #Layer 2
        x = self.layer1_bak(x)
        x_diff = func.interpolate(x_diff, x.size()[2:])
...
def net(base_model=None,num_segments=8,pretrained=True, **kwargs):
    ...
    if(num_segments is 8):
        model = Net(resnet_model,resnet_model1,apha=0.5,belta=0.5)
    else:
        model = Net(resnet_model,resnet_model1,apha=0.75,belta=0.25)
    return model